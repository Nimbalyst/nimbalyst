...
- The initial-setup mode picker no longer hangs with a beach ball when the `onboarding:get` IPC call stalls. Two `useEffect` hooks (one in `UnifiedOnboarding.tsx`, one in `useOnboarding.ts`) fire the same channel on dialog open. The main-process handler uses a dynamic `await import('../utils/store')` that can stall on cold start, and if either renderer-side promise never resolves the dialog renders its initial "no mode selected" state forever, with no interactivity, requiring a force-quit. Both call sites now race the IPC against a 3-second timeout and fall back to the conservative new-user / show-dialog path on timeout. The dialog stays interactive even if the main process is wedged. Fixes #260.
