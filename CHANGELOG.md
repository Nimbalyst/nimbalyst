...
- Detect frontmatter on Windows files with CRLF line endings. Three frontmatter regexes (`extractFrontmatter` and `updateFrontmatter` in `packages/runtime/src/plugins/TrackerPlugin/documentHeader/frontmatterUtils.ts`, plus `FRONTMATTER_REGEX` in `packages/extensions/automations/src/frontmatter/parser.ts`) required bare `\n` after `---`, so files checked out with `git config core.autocrlf=true` were silently undetectable to the Tracker view, the automations daemon, and `parseAutomationStatus`. All three regexes now use `\r?\n`. The main-process bounded reader (`frontmatterReader.ts`) already tolerated CRLF and is unchanged. Adds CRLF round-trip tests in both packages. Fixes #68.
