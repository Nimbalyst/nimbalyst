...
- Pass target platform/arch to the packaged-SDK validator: the `afterPack` validator was inferring the build target from `appOutDir` and silently falling back to the host arch when no arch token was present (e.g. `release/mac/` for the default-arch Mac build). On the cross-arch x64 Mac job that runs on an arm64 runner, the validator then checked the host arm64 claude binary -- which `afterPack` had correctly pruned for being non-target -- and failed the build for a package that was deliberately and correctly absent. `afterPack` now passes `--platform`/`--arch` from electron-builder's context, the authoritative source; the validator prefers the flags and refuses to fall back to the host arch silently when both flags and a path-derived arch are missing.
- Install codex-acp cross-arch binary for Mac x64 builds (fixes v0.58.7 + v0.58.8 build failures): the cross-arch install step explicitly installed `@anthropic-ai/claude-agent-sdk` and `@openai/codex` platform packages for the target arch but skipped `@zed-industries/codex-acp`, so on the arm64 runner cross-compiling the x64 Mac build, `@zed-industries/codex-acp-darwin-x64` was never on disk and `validate-packaged-sdks` correctly failed at `afterPack`. Adds `@zed-industries/codex-acp-${NPM_PLATFORM}-${TARGET_ARCH}` to the cross-arch install in `electron-build.yml` alongside the existing claude/codex installs, and a matching `mac.extraResources` entry for `@zed-industries/codex-acp-darwin-${arch}`.
