  ...
  useEffect(() => {
    const setContent = async () => {
      try {
        ...
        // Ignore worker errors that fall back to main thread
        if (error instanceof Error && error.message.includes('window is not defined')) {
          console.warn('Excalidraw font subsetting fell back to main thread');
        } else {
          console.error('Error exporting Excalidraw to SVG:', error);
        }
      }
    };
    setContent();
