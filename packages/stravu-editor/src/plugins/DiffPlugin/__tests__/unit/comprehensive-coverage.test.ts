    ...
    describe('Large table structures', () => {
      it('should handle tables with many rows and columns (50x10)', () => {
        ...
        for (let r = 1; r <= 10; r++) {
          ...
          const targetRow = [];
          for (let c = 1; c <= 5; c++) {
            const cellValue = `R${r}C${c}`;
            row.push(cellValue);
            // Modify cells in row 5
            targetRow.push(r === 5 ? `MODIFIED_${cellValue}` : cellValue);
          }
          original += `| ${row.join(' | ')} |\n`;
          target += `| ${targetRow.join(' | ')} |\n`;
        }

        const result = setupMarkdownDiffTest(original.trim(), target.trim());

        expect(result.diff).toContain('MODIFIED_R5C1');
        expect(result.getApprovedMarkdown()).toContain('MODIFIED\\_R5C1');
      });
    });
...
- Add new root unordered`;
  ...
  describe('Table Complex Scenarios', () => {
    // Tests for advanced table operations
