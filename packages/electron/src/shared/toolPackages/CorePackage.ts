...
export const CorePackage: ToolPackage = {
  id: 'core',
  name: 'Core',
  description: 'Essential planning, tracking, and wireframe commands for all workflows',
  icon: 'verified',
  version: '1.0.0',
  author: 'Nimbalyst',
...
`,
    ...
    {
      name: 'mockup',
      description: 'Create a UX wireframe mockup using WireframeLM',
      content: `---
packageVersion: 1.0.0
packageId: core
---

Create a visual UX wireframe mockup for: {{arg1}}
...
2. **Check for style guide** - Look for \`nimbalyst-local/existing-screens/style-guide.wireframe.html\`
   - **If style guide DOES NOT EXIST**:
     - Use the Task tool to spawn a sub-agent that will:
       ...
       - Create \`nimbalyst-local/existing-screens/style-guide.wireframe.html\` - a comprehensive visual reference showing:
...
3. **Create mockup file** - Create \`nimbalyst-local/mockups/[descriptive-name].wireframe.html\`
...
2. **Check for cached replica** - Look in \`nimbalyst-local/existing-screens/\` for \`[screen-name].wireframe.html\`
...
4. **If cached replica DOES NOT EXIST**:
   ...
   - Create \`nimbalyst-local/existing-screens/[screen-name].wireframe.html\` - a **pixel-perfect** HTML/CSS replica including:
...
5. **Copy to mockups** - Copy the existing-screen replica to \`nimbalyst-local/mockups/[descriptive-name].wireframe.html\`
...
- Use kebab-case: \`settings-page.wireframe.html\`, \`checkout-flow.wireframe.html\`
- Always use \`.wireframe.html\` extension
...
The user can draw on wireframes (circles, arrows, highlights). These annotations are **NOT** in the HTML source - you can only see them by capturing a screenshot with \`mcp__nimbalyst-mcp__capture_wireframe_screenshot\`.

When the user draws annotations:
1. Capture a screenshot to see what they marked
2. Interpret their feedback
3. Update the wireframe accordingly
...
- **Can't find existing screen**: Ask the user to clarify which screen they mean, or offer to create a new wireframe instead
- **Complex multi-screen flow**: Offer to create separate wireframe files for each screen
