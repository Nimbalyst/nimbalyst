...
test.describe('Tab Content Isolation', () => {
  ...
  test('should preserve each file content independently when switching tabs', async () => {
    ...
    await expect(page.locator(ACTIVE_FILE_TAB_SELECTOR)).toContainText('gamma.md', { timeout: TEST_TIMEOUTS.TAB_SWITCH });
    await page.waitForTimeout(TEST_TIMEOUTS.DEFAULT_WAIT);
    ...
    await page.locator('.file-tabs-container .tab', { has: page.locator('.tab-title', { hasText: 'beta.md' }) }).click();
    await expect(page.locator(ACTIVE_FILE_TAB_SELECTOR)).toContainText('beta.md', { timeout: TEST_TIMEOUTS.TAB_SWITCH });
    await page.waitForTimeout(TEST_TIMEOUTS.DEFAULT_WAIT);
  ...
  test('should handle rapid tab switching without content corruption', async () => {
    ...
    await page.locator('.file-tree-name', { hasText: 'alpha.md' }).click();
    await expect(page.locator('.file-tabs-container .tab .tab-title', { hasText: 'alpha.md' })).toBeVisible({ timeout: TEST_TIMEOUTS.TAB_SWITCH });

    await page.locator('.file-tree-name', { hasText: 'beta.md' }).click();
    await expect(page.locator('.file-tabs-container .tab .tab-title', { hasText: 'beta.md' })).toBeVisible({ timeout: TEST_TIMEOUTS.TAB_SWITCH });

    await page.locator('.file-tree-name', { hasText: 'gamma.md' }).click();
    await expect(page.locator('.file-tabs-container .tab .tab-title', { hasText: 'gamma.md' })).toBeVisible({ timeout: TEST_TIMEOUTS.TAB_SWITCH });
    ...
    // Gamma
    await page.locator('.file-tabs-container .tab', { has: page.locator('.tab-title', { hasText: 'gamma.md' }) }).click();
  ...
  test('should auto-save on tab switch to prevent data loss', async () => {
    ...
    const betaTab = page.locator('.file-tabs-container .tab', { has: page.locator('.tab-title', { hasText: 'beta.md' }) });
