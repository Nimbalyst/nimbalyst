...
| Data 3 | Data 4 |`
  ...
  test('should individually test table deletion', async () => {
    console.log('\n=== Testing table deletion only ===');
...
| Cell 3 | Cell 4 |`,
    ...
    console.log('Markdown after delete:', markdown);
    expect(markdown.includes('## Table to delete')).toBe(false);

    // Check if the table has the diffRemove class applied
    const tableDiffClasses = await page.evaluate(() => {
      const editor = document.querySelector('.multi-editor-instance.active .editor');
      const tables = Array.from(editor?.querySelectorAll('table') || []);
      return tables.map(table => ({
        hasRemoveClass: table.classList.contains('PlaygroundEditorTheme__diffRemove'),
        hasAddClass: table.classList.contains('PlaygroundEditorTheme__diffAdd'),
        classes: Array.from(table.classList)
      }));
    });

    console.log('Table diff classes:', JSON.stringify(tableDiffClasses, null, 2));

    // At least one table should have the diffRemove class (the deleted table)
    const hasRemovedTable = tableDiffClasses.some(t => t.hasRemoveClass);
    console.log('Has table with diffRemove class:', hasRemovedTable);
    expect(hasRemovedTable).toBe(true);
  });

  test('should individually test table editing', async () => {
    console.log('\n=== Testing table edit only ===');
...
| Charlie | 35 | SF |`
    ...
    console.log('Markdown after edit:', markdown);
    expect(markdown.includes('| Charlie | 35 | SF |')).toBe(true);
  });

  test('should individually test table addition', async () => {
    console.log('\n=== Testing table addition only ===');
...
| Data 3 | Data 4 |`
    ...
    console.log('Markdown after add:', markdown);
    expect(markdown.includes('| Column A | Column B |')).toBe(true);
  });
});
