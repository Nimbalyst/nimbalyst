...
export class ClaudeCodeProvider extends BaseAgentProvider {
  ...
  async *sendMessage(
    ...
    try {
      ...
      // measure the delay between result and any subsequent "Stream closed"
      // tool permission error -- a small delta confirms the binary is making
      // post-result can_use_tool calls after stdin is closed.
      ...
      try {
        ...
        while (true) {
          ...
          // Diagnostic: capture when the binary emits `type: 'result'` -- the
          // signal the SDK uses to close stdin on single-turn queries. We now
          // force isSingleUserTurn=false via the persistent prompt stream, so
          // stdin should NOT close here, but we log it to verify the fix and
          // to measure the delta to any subsequent STREAM_CLOSED_DIAGNOSTIC.
          if (typeof chunk === 'object' && chunk !== null && chunk.type === 'result' && resultReceivedTime === null) {
      ...
      // returns and closes the binary's stdin pipe cleanly. Safe to call
      // multiple times -- idempotent inside the controller.
  ...
  abort(): void {
    ...
    // End the prompt stream so the SDK can close stdin; the finally block in
    // sendMessage will also null this, but aborts can fire before we re-enter
    // the finally (e.g. during a for-await iteration).
    if (this.promptController) {
